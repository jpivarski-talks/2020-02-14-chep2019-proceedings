...
The nodes that define the structure of an array, which were user-level types in the original Awkward Array library, are accessible through a \mintinline{python}{layout} property, but most physicists won't need to access this layer.
...
Instead, this feature is implemented in Awkward 1.0 by only applying the interpretation when creating the high-level wrapper, and keeping track of how to interpret each \mintinline{python}{layout} node with \mintinline{python}{parameters} that pass through C++ and Numba. For example,
...
The structural nodes underlying the above example are all C++ classes, wrapped in Python using pybind11 [ref]. The nesting of structural nodes for this example is presented in Figure~\ref{fig:example-hierarchy}. Each node is a \mintinline{c++}{std::shared_ptr<Content>}, where \mintinline{c++}{Content} is the superclass of all node classes like \mintinline{c++}{ListOffsetArray64}, \mintinline{c++}{RecordArray}, and \mintinline{c++}{NumpyArray}. Operations on arrays are virtual methods, implemented for all classes. Compile-time templates are only used to specialize integer types (e.g.\ \mintinline{c++}{ListOffsetArray32} versus \mintinline{c++}{ListOffsetArray64}), not for nesting structures: it is necessary to dynamically combine types to use them in Python.
